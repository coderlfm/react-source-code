...
function sholdUpdate(classInstance, nextProps) {
  ...
  if (classInstance.shouldComponentUpdate && !classInstance.shouldComponentUpdate(classInstance.props, classInstance.update.getState())) {
...
export class PureComponent extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    // 浅层比较 state
    const nextStateLength = Object.keys(nextState).length;
    const prevStateLength = Object.keys(this.state).length;

    if (nextStateLength !== prevStateLength) return true;
    ...
    const prevPropsLength = Object.keys(this.props).length;

    if (nextPropsLength !== prevPropsLength) return true;
